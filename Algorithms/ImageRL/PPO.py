...
class ImagePPO:#make it useful for both discrete(cartegorical actor) and continuous actor(gaussian policy)
    def __init__(self, obs_dim, action_dim, args):
        ...
        if self.discrete == True:
            self.actor = Policy_network(self.feature_dim, self.action_dim, args.hidden_dim, kernel_initializer='RandomUniform')
        else:
            self.actor = Gaussian_Actor(self.feature_dim, self.action_dim, args.hidden_dim, kernel_initializer='RandomUniform')

        self.critic = V_network(self.feature_dim)
        self.encoder = PixelEncoder(self.obs_dim, self.feature_dim, self.layer_num, self.filter_num, data_format='channels_last')
