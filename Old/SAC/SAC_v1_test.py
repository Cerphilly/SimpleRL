...
#https://gist.github.com/dc91dfc35c9f6469a942569e336fdfe7
...
class Q_network(tf.keras.Model):
    def __init__(self, state_dim, hidden_units, action_dim):
        ...
        self.input_layer = tf.keras.layers.Dense(hidden_units[0], kernel_initializer='RandomNormal')

        self.hidden_layers = []
        for i in range(len(hidden_units) - 1):
            self.hidden_layers.append(tf.keras.layers.Dense(hidden_units[i+1], kernel_initializer='RandomNormal'))
...
class Policy_network(tf.keras.Model):
    def __init__(self, state_dim, hidden_units, action_dim):
        ...
        self.output_layer = tf.keras.layers.Dense(self.action_dim*2)
...
class SAC:
    def __init__(self, state_dim, action_dim, max_action, min_action, save, load, batch_size=100, tau=0.995, learning_rate=0.0003, gamma=0.99, alpha=0.2, reward_scale=1):
        ...
        self.saver = Saver([self.actor, self.critic1, self.critic2, self.v_network, self.target_v_network], ['actor', 'critic1', 'critic2', 'v_network', 'target_v_network'], self.buffer,
                           '/home/cocel/PycharmProjects/SimpleRL/SAC/SAC_v1_3pole_balance_copy')
...
if __name__ == '__main__':
    ...
    sac = SAC(state_dim, action_dim, max_action, min_action, False, True)
    sac.run()
