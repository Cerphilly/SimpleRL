...
class Reward_Network(tf.keras.Model):
    def __init__(self, feature_dim, hidden_dim=256, kernel_initializer='glorot_uniform', bias_initializer='zeros'):
        ...
        self.hidden_layers = []
        self.hidden_layers.append(tf.keras.layers.Dense(hidden_dim, kernel_initializer=kernel_initializer, bias_initializer=bias_initializer, name="dense1"))
...
class Transition_Network(tf.keras.Model):
    def __init__(self, feature_dim, action_dim, hidden_dim=256, deterministic=False, log_std_min=-10, log_std_max=2,
        ...
        self(tf.zeros(shape=(1,) + (self.feature_dim,), dtype=tf.float32), tf.zeros(shape=(1,) + (self.action_dim,), dtype=tf.float32))
